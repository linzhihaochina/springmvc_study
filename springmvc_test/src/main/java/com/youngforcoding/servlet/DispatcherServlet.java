...
import com.youngforcoding.pojo.Handler;
...
//        method.invoke() //
        ...
        if (handler.getAllowUserNames() != null && !handler.getAllowUserNames().isEmpty()) {
            if (!parameterMap.containsKey("username")) {
                resp.getWriter().println("not allow!!!");
                return;
            }
            String[] usernameValues = parameterMap.get("username");
            boolean find = false;
            for (String usernameValue : usernameValues) {
                if (handler.getAllowUserNames().contains(usernameValue)) {
                    find = true;
                    break;
                }
            }
        ...
        for (Map.Entry<String, String[]> param : parameterMap.entrySet()) {
            ...
            String value = String.join(",", param.getValue());  // 如同 1,2
        ...
        try {
            resp.getWriter().println(handler.getMethod().invoke(handler.getController(), paraValues));
    ...
    private Handler getHandler(HttpServletRequest req) {
        ...
        for (Handler handler : handlerMapping) {
            ...
            if (handler.getMethod().isAnnotationPresent(Security.class)) {
                Security declaredAnnotation = handler.getMethod().getDeclaredAnnotation(Security.class);
                handler.getAllowUserNames().addAll(Arrays.asList(declaredAnnotation.value()));
            }
            return handler;
        }
